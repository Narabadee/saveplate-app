...
export default function BookingDetail({ bag, onBack, onAddOrder }) {
    ...
    return (
        <>
                ...
                <div className="px-5 -mt-4 space-y-4 pb-32">
                    ...
                    <div className="bg-white dm-card rounded-2xl shadow-sm border border-gray-100 p-5 page-fade-in" style={{ animationDelay: '0.2s' }}>
                        ...
                        <div className="space-y-3">
                            ...
                            {/* Reviews Section */}
                            <div className="bg-white dm-card rounded-2xl shadow-sm border border-gray-100 p-5 page-fade-in" style={{ animationDelay: '0.3s' }}>
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="font-bold text-gray-800 dm-text">What others say</h3>
                                    <span className="text-xs text-emerald-600 font-semibold bg-emerald-50 px-2 py-1 rounded-lg">
                                        {MOCK_REVIEWS.filter(r => r.storeId === bag.id).length} reviews
                                    </span>
                                </div>
                                ...
                                {MOCK_REVIEWS.filter(r => r.storeId === bag.id).length > 2 && (
                                    <button
                                        ...
                                        className="w-full mt-3 py-2 text-sm text-emerald-600 font-semibold bg-emerald-50 rounded-xl hover:bg-emerald-100 transition-colors"
                                    >
                                        {showAllReviews ? 'Show Less' : `Show All ${MOCK_REVIEWS.filter(r => r.storeId === bag.id).length} Reviews`}
                                    </button>
                                )}
                            </div>
                        </div>
            ...
            {/* Sticky CTA Footer - Outside transformed container */}
            <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white dm-card p-5 border-t border-gray-100 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-50">
